...
ViewMainWindow::ViewMainWindow(bool manager, QWidget *parent):
    DMainWindow(parent)
{
...
void ViewMainWindow::resizeEvent(QResizeEvent *e)
{
    if (! isMaximized()
...
bool ViewMainWindow::windowAtEdge()
{
    ...
    if (currentRect.x() == 0 || qAbs(currentRect.right() - dApp->primaryScreen()->geometry().width()) <= 5) {
